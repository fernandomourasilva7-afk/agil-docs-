...
export default async function ClienteDetalhePage({ params }: Props) {
  ...
  return (
    <div className="px-4 py-6 lg:px-8">
      {/* Breadcrumb */}
      <Link href="/dashboard" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-600 mb-5 transition-colors">
      ...
      <Card className="mb-6 border-teal-200 bg-teal-50">
        <CardContent className="pt-4 pb-4 px-5">
          <p className="text-sm font-medium text-teal-800 mb-2">Link do cliente</p>
          <div className="flex items-center gap-2 flex-wrap">
            <code className="text-sm bg-white border border-teal-200 rounded px-3 py-1.5 text-teal-700 flex-1 min-w-0 truncate">
              /portal/{c.slug}
            </code>
            <CopiarLink slug={c.slug} />
            <a href={`/portal/${c.slug}`} target="_blank">
              <Button size="sm" variant="outline" className="gap-1.5 border-teal-300 text-teal-700 hover:bg-teal-100">
                Abrir
              </Button>
            </a>
          </div>
